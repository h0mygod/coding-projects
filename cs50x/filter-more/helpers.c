...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int y = 0; y < height; y++)
    {
        ...
        {
            int brightness = round((image[y][x].rgbtRed + image[y][x].rgbtBlue + image[y][x].rgbtGreen) / 3.0);
            image[y][x].rgbtRed = brightness;
            image[y][x].rgbtBlue = brightness;
            image[y][x].rgbtGreen = brightness;
        }
    }
    return;
}
